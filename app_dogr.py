...
st.set_page_config(page_title="Dogri ASR + Translation", layout="wide")
st.title("Dogri ASR + Translation")
st.caption("Upload/record Dogri speech → Dogri transcript + English translation")

st.markdown("---")

# ---------------- Audio input ----------------
st.subheader("1) Provide Dogri audio")
...
if input_method == "Record with microphone":
    audio_file = st.audio_input(
        "Click to record your Dogri audio, then click again to stop:",
        key="audio_rec",
    )
    ...
    uploaded_file = st.file_uploader(
        "Upload a .wav file with Dogri audio:",
        type=["wav"],
        key="audio_upload",
    )
...
with col_btn:
    if st.button("Run Dogri ASR", type="primary"):
        ...
        filename = "streamlit_Dogri_{}.wav".format(timestamp_str)
...
if not result:
    st.info("Click **Run Dogri ASR** to get transcript + translation.")
    st.stop()
